  ...
  if (!!text.length) {
    ...
    $.ajax({
      ...
      error: function() {
        that
          .removeAttr("disabled", "disabled")
          .removeClass("is-loading is-dark")
          .addClass("is-warning")
          .text("Click to retry!");
      },
      success: function(data) {
        ...
        comment
          .find(".comment-annotationid")
          .attr("data-annotationid", data["annotation_id"]);
        comment
          .find(".comment-annotationid")
          .attr("data-tweetid", data["tweet_id"]);
        comment
          .find(".comment-annotationid")
          .attr("data-storyid", data["story_id"]);
        comment.find(".comment-ts").text(data["timestamp"]);
        that
          .closest(".finder-block")
...
$(document).on("click", ".rfc-trigger", ev => {
  ...
  let that = $(ev.currentTarget);
  if (
    confirm("Are you sure you want to remove this story from this collection?")
  ) {
    let collection_id = that.data("collectionid");
    let payload = {
      action: "REMOVE",
